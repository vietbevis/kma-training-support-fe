...
const VisitingLecturersActivePageComponent = () => {
  ...
  const { data, isLoading, isFetching } = useGetVisitingLecturersQuery({
    ...
    academyApproved: true // Force true cho trang này
  })
  ...
  return (
    <div className='space-y-6'>
      <div className='flex justify-between items-center'>
        <div>
          <h1 className='text-3xl font-bold tracking-tight'>Giảng viên mời đang giảng dạy</h1>
          <p className='text-muted-foreground'>
            Danh sách giảng viên mời đang trong quá trình giảng dạy ({visitingLecturers.length} giảng viên mời)
          </p>
        </div>
        <PermissionButton
          asChild
      ...
      <VisitingLecturerFilters filters={filters} setFilters={setFilters} resetFilters={resetFilters} page='active' />

      <Card>
        <CardHeader>
          <CardTitle>Danh sách giảng viên mời đang giảng dạy ({meta?.total || 0})</CardTitle>
        </CardHeader>
        <CardContent>
...
export const VisitingLecturersActivePage = withPermissionGuard(
  VisitingLecturersActivePageComponent,
